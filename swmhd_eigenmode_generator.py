...
def get_analytical_kelvin_fields(Oh, m_val, r_g, target_max_u0):
    ...
    # Compute using shift-and-exponentiate technique in log-space to prevent underflow/overflow
    log_eta = (m_val / ws) * np.log(r_g) + exponent * (0.5 * r_g**2 - r_g)
    log_eta_max = np.max(log_eta)
    eta = np.exp(log_eta - log_eta_max)

    eta_max_val = np.max(np.abs(eta))
    if eta_max_val > 0:
        eta /= eta_max_val
...
def create_standalone_plot(Oh, eta, m_val, label, filename, eta_lim, u0_lim, target_max_u0, show_ticks, inset_loc, cmap='viridis'):
    ...
    eta_max = np.max(np.abs(ETA2D))
    if eta_max < np.finfo(float).eps:
        eta_max = np.finfo(float).eps
