...
def chebyshev_lobatto(N, r_min, r_max):
    ...
    Returns
    -------
    r  : (N,) array of collocation points, r[0]=r_min, r[N-1]=r_max
...
def P_coeff(hat_r):
    """Coefficient of d_eta/d_hat_r in the wave ODE."""
    return 1.0 / hat_r - (1.0 + gamma) * (hat_r - 1.0) / hat_c0sq

def Q_coeff(hat_r, hat_omega):
    """Coefficient of eta in the wave ODE."""
    ws_hat = omega_star_hat(hat_omega)
    term1 = 4.0 * hat_omega * (ws_hat**2 - 1.0) / (ws_hat * hat_c0sq)
    term2 = -m**2 / hat_r**2
    term3 = -(1.0 + gamma) * (2.0 * hat_r - 1.0) / (hat_c0sq * hat_r)
    term4 = -m * (1.0 + gamma) * (hat_r - 1.0) / (ws_hat * hat_c0sq * hat_r)
    return term1 + term2 + term3 + term4

# =============================================================================
# Build the collocation matrix for a given hat_omega
# =============================================================================
def build_matrix(hat_omega):
    ...
    """
    ws_hat = omega_star_hat(hat_omega)
    P_vec = P_coeff(hat_r_grid)
    Q_vec = Q_coeff(hat_r_grid, hat_omega)

    L = D2 + np.diag(P_vec) @ D1 + np.diag(Q_vec)

    for idx in [0, N - 1]:
        hat_r_bc = hat_r_grid[idx]
        bc_eta_coeff = -(ws_hat * (1.0 + gamma) * (hat_r_bc - 1.0) + m * hat_c0sq / hat_r_bc)
...
# =============================================================================
def det_L(hat_omega):
    return det(build_matrix(hat_omega))
...
hat_omega_max_scan = 3.0 * max(hat_omega_inertial, hat_omega_gravity, hat_omega_alfven)
hat_omega_min_scan = -hat_omega_max_scan
...
if len(roots) > 0:
    ...
    for k, wr in enumerate(sorted(roots, key=lambda x: x.real)):
        ...
        for r_bc, label in [(hat_r1, 'r1'), (hat_r2, 'r2')]:
            ...
            vr_bc   = (ws_hat * hat_c0sq * deta_bc
                       - (ws_hat * (1.0 + gamma) * (r_bc - 1.0)
                          + m * hat_c0sq / r_bc) * eta_bc)
