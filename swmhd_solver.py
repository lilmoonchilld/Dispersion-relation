...
# Caches and state for find_eigenvalues
_matrix_cache = {}
_last_solved_m = None
_last_solved_eigs = None

def get_sigma_min(hat_omega, m_val):
    key = (float(hat_omega), int(m_val), int(N_col))
    ...
    try:
        sigmas = svdvals(L)
        val = float(np.min(sigmas))
    except Exception:
        val = np.nan
    _matrix_cache[key] = val
    return val

def find_eigenvalues(m_val, omega_range=(-45, 45), n_scan=1000, return_eigenfunctions=False):
    """
    Return sorted real eigenfrequencies (dimensionless) for azimuthal mode m_val,
    using a singular-value-based nonlinear eigenvalue solver.
    """
    global _last_solved_m, _last_solved_eigs
    tol = 1e-4  # Using 1e-4 because singular values scale with matrix size/Chebyshev order and are not exactly zero

    # 1. Try Continuation first if we have the previous mode's eigenvalues
    roots = []
    continuation_successful = False

    if _last_solved_m is not None and m_val == _last_solved_m + 1 and _last_solved_eigs is not None and len(_last_solved_eigs) > 0:
        continuation_failed = False
        temp_roots = []
        for w_guess in _last_solved_eigs:
            half_width = 1.5
            # Local grid search around guess
            local_scan = np.linspace(w_guess - half_width, w_guess + half_width, 21)
            local_sigmas = [get_sigma_min(w, m_val) for w in local_scan]

            # Find local minima
            min_indices = []
            for k in range(1, len(local_sigmas) - 1):
                if local_sigmas[k] < local_sigmas[k-1] and local_sigmas[k] < local_sigmas[k+1]:
                    min_indices.append(k)

            if len(min_indices) > 0:
                min_indices.sort(key=lambda idx: local_sigmas[idx])
                best_idx = min_indices[0]
                try:
                    from scipy.optimize import minimize_scalar
                    res = minimize_scalar(
                        lambda w: get_sigma_min(w, m_val),
                        bracket=(local_scan[best_idx-1], local_scan[best_idx], local_scan[best_idx+1]),
                        method='brent',
                        options={'xtol': 1e-12}
                    )
                    if res.success and res.fun < tol:
                        w_refined = res.x
                        if not any(abs(w_refined - r) < 5e-3 for r in temp_roots):
                            temp_roots.append(w_refined)
                        else:
                ...
                # Direct bounded minimization if no local minimum was resolved on grid
                try:
                    from scipy.optimize import minimize_scalar
                    res = minimize_scalar(
                        lambda w: get_sigma_min(w, m_val),
                        bounds=(w_guess - half_width, w_guess + half_width),
                        method='bounded',
                        options={'xatol': 1e-12}
                    )
                    if res.success and res.fun < tol:
        ...
        if not continuation_failed and len(temp_roots) == len(_last_solved_eigs):
            roots = temp_roots
            continuation_successful = True

    # 2. Global Coarse Scan if continuation is not attempted or fails
    if not continuation_successful:
        roots = []
        ...
        ld = []
        for w in scan:
            ld.append(get_sigma_min(w, m_val))
        ...
        for k in range(1, len(ld) - 1):
            if np.isfinite(ld[k]) and ld[k] < ld[k-1] and ld[k] < ld[k+1]:
                try:
                    ...
                    res = minimize_scalar(
                        lambda w: get_sigma_min(w, m_val),
                        bracket=(scan[k-1], scan[k], scan[k+1]),
                        method='brent',
                        options={'xtol': 1e-12}
                    )
                    if res.success and res.fun < tol:
                        w_refined = res.x
                        dup = any(abs(w_refined - wr) < 5e-3 for wr in roots)
                        if not dup:
                            roots.append(w_refined)
                except Exception:
                    pass

    sorted_roots = np.array(sorted(roots))

    _last_solved_m = m_val
    _last_solved_eigs = sorted_roots

    if return_eigenfunctions:
        from scipy.linalg import svd
        eigenfunctions = []
        for w in sorted_roots:
            L = build_matrix(w, m_val)
            try:
                U, S, Vh = svd(L)
                eta = Vh[-1, :]
                eta = eta / np.max(np.abs(eta))
            except Exception:
                eta = np.zeros_like(hat_r_grid)
            eigenfunctions.append(eta)
        return sorted_roots, eigenfunctions
    else:
        return sorted_roots
...
def validate_solver(m_list=[1, 2, 3]):
    global N_col, hat_r_grid, D1_mat, D2_mat

    orig_N_col = N_col
    orig_r_grid = hat_r_grid
    orig_D1 = D1_mat
    orig_D2 = D2_mat
    ...
    for N in resolutions:
        print(f"\n--- Validating solver at resolution N = {N} ---")
        N_col = N
        hat_r_grid, D1_mat, D2_mat = chebyshev_lobatto(N, hat_r1, hat_r2)

        # Reset continuation state to avoid cross-resolution interference
        global _last_solved_m, _last_solved_eigs
        _last_solved_m = None
        _last_solved_eigs = None
        ...
        results_by_N[N] = res_m
        cpu_times[N] = t1 - t0
        print(f"Completed N = {N} in {cpu_times[N]:.2f} seconds.")

    N_col = orig_N_col
    hat_r_grid = orig_r_grid
    D1_mat = orig_D1
    D2_mat = orig_D2
    _matrix_cache.clear()
    _last_solved_m = None
    ...
    def flatten_results(res_m):
        flat = {}
        for m, assigned in res_m.items():
            if assigned['MK_in'] is not None:
                flat[(m, 'MK_in')] = assigned['MK_in']
            if assigned['MK_out'] is not None:
                flat[(m, 'MK_out')] = assigned['MK_out']
            for n, val in assigned['MP_p'].items():
                flat[(m, f'MP_p_n{n}')] = val
            for n, val in assigned['MP_m'].items():
                flat[(m, f'MP_m_n{n}')] = val
    ...
    flat_64 = flatten_results(results_by_N[64])
    flat_128 = flatten_results(results_by_N[128])
    flat_256 = flatten_results(results_by_N[256])

    def compare_flat(flat_ref, flat_comp):
        keys_ref = set(flat_ref.keys())
        keys_comp = set(flat_comp.keys())

        missing = keys_ref - keys_comp
        additional = keys_comp - keys_ref
        common = keys_ref & keys_comp
        ...
        for k in common:
            ...
            v_comp = flat_comp[k]
            if abs(v_ref) > 1e-12:
                rel_err = abs(v_comp - v_ref) / abs(v_ref)
            else:
                rel_err = abs(v_comp - v_ref)
            rel_errors.append(rel_err)

        if rel_errors:
            max_rel = np.max(rel_errors)
            rms_rel = np.sqrt(np.mean(np.array(rel_errors)**2))
        else:
            max_rel, rms_rel = 0.0, 0.0
    ...
    comp_64_128 = compare_flat(flat_64, flat_128)
    comp_128_256 = compare_flat(flat_128, flat_256)

    print("=" * 60)
    print("               SWMHD SOLVER VALIDATION REPORT")
    print("=" * 60)
    print(f"Modes validated: {m_list}")
    print(f"CPU Times:")
    for N in resolutions:
        print(f"  N = {N:3d}: {cpu_times[N]:.2f} seconds")
    print("-" * 60)
    print("Resolution N=64 vs N=128:")
    print(f"  Max Relative Difference: {comp_64_128['max_rel']:.2e}")
    print(f"  RMS Relative Difference: {comp_64_128['rms_rel']:.2e}")
    print(f"  Missing modes in N=128:  {len(comp_64_128['missing'])} {list(comp_64_128['missing'])}")
    print(f"  Extra modes in N=128:    {len(comp_64_128['additional'])} {list(comp_64_128['additional'])}")
    print("-" * 60)
    print("Resolution N=128 vs N=256:")
    print(f"  Max Relative Difference: {comp_128_256['max_rel']:.2e}")
    print(f"  RMS Relative Difference: {comp_128_256['rms_rel']:.2e}")
    print(f"  Missing modes in N=256:  {len(comp_128_256['missing'])} {list(comp_128_256['missing'])}")
    print(f"  Extra modes in N=256:    {len(comp_128_256['additional'])} {list(comp_128_256['additional'])}")
    print("=" * 60)

# ── 5.  Plotting and full sweep driver ────────────────────────────────────────
def run_full_sweep_and_plot():
    M_max = 30
    ...
    # Save statements kept commented out as requested
    ...
    print("\n[Saved statement] swmhd_four_branch_dispersion.pdf / .png (kept commented out)")
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) > 1 and "validate" in sys.argv[1].lower():
        # Validate for first few modes (e.g., m=1, 2)
        validate_solver(m_list=[1, 2])
    else:
        run_full_sweep_and_plot()
