...
def get_analytical_kelvin_fields(Oh, m_val, r_g, target_max_u0):
    ...
    # Use log-space subtraction to prevent numerical underflow/overflow of exponents
    log_eta = (m_val / ws) * np.log(r_g) + exponent * (0.5 * r_g**2 - r_g)
    eta = np.exp(log_eta - np.max(log_eta))
...
def create_standalone_plot(Oh, eta, m_val, label, filename, eta_lim, u0_lim, target_max_u0, show_ticks, inset_loc, cmap='viridis'):
    ...
    # Add diagnostic warning and normalize cmap symmetrically to the absolute max value of ETA2D
    if np.max(np.abs(ETA2D)) < 1e-12:
        print(f"Warning: Nearly-zero ETA2D field for {label}")

    eta_max = float(np.max(np.abs(ETA2D)))
    eta_max = max(eta_max, 1e-12)
