...
# Let's set some non-zero values
B0    = 0.05          # T
C     = 1.5e9         # m^3/s^2
...
def find_eigenvalues(m_val, hat_r_grid, D1_mat, D2_mat, gamma, hat_c0sq, omega_range=(-45, 45), n_scan=1500):
...
for x in x_vals:
    ...
    for k in k_modes:
        eigs = find_eigenvalues(k, hat_r_grid, D1_mat, D2_mat, gamma, hat_c0sq, omega_range=(-3, 3), n_scan=800)

        # WKB predictions
        arg_in = k**2 * hat_c0sq / hat_r1**2 - 4.0 * hat_omA2
        oMK_in = -0.5 * np.sqrt(arg_in) if arg_in >= 0 else np.nan

        arg_out = k**2 * hat_c0sq / hat_r2**2 - 4.0 * hat_omA2
...
for k in [1, 3, 5]:
    ...
    # WKB Theory lines
    arg_in_th = (k * 1.5 / x_fine)**2 - 4 * hat_omA**2
    valid_in = arg_in_th >= 0
    sigma_in = np.full_like(x_fine, np.nan)
    sigma_in[valid_in] = -0.5 * np.sqrt(arg_in_th[valid_in])

    arg_out_th = (k * 0.75 / x_fine)**2 - 4 * hat_omA**2
    valid_out = arg_out_th >= 0
    sigma_out = np.full_like(x_fine, np.nan)
    sigma_out[valid_out] = 0.5 * np.sqrt(arg_out_th[valid_out])

    ax.plot(x_fine, sigma_out, color=color, ls=ls, lw=2.0)
    ax.plot(x_fine, sigma_in, color=color, ls=ls, lw=2.0)

    # Collocation dots
    ax.scatter(x_vals, num_out_data[k], color='k', s=15, zorder=5)
    ax.scatter(x_vals, num_in_data[k], color='k', s=15, zorder=5)
...
plt.tight_layout()
plt.savefig('kelvin_modes_numerical.pdf', dpi=300)
plt.savefig('kelvin_modes_numerical.png', dpi=300)
print("Saved")
